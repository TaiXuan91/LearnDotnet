...
这里我们只介绍Identifiers的声明。我们之前介绍的，命名空间的定义，实际上，就是声明了命名空间的名称。而类定义本身实际上就声明了类名。对于函数成员的定义，实际上也是声明了这些成员名。例如定义了一个函数，就声明了这个函数的函数名。但是对于变量和常量的说明需要特别说明一下。它们可以在类定义中或者函数定义中声明。声明是一种语句，其的一般结构是[类型] [名称]。例如：
